...
export default function ClientPage() {
  ...
  const getMyRequests = async () => {
    ...
    const res = await axios.get("http://localhost:5000/api/lesson-requests/my", {
      headers: { token }
    });
  ...
  const getMyLessons = async () => {
    ...
    const res = await axios.get("http://localhost:5000/api/lessons/client/my", {
      headers: { token }
    });
  ...
  const getMyChangeRequests = async () => {
    ...
    const res = await axios.get("http://localhost:5000/api/change-requests/my", {
      headers: { token }
    });
  ...
  const sendChangeRequest = async (lessonId) => {
    try {
      if (!requestedDate || !requestedTime || !changeReason) {
        alert("Popuni datum, vreme i razlog promene.");
        return;
      }
      ...
      alert("Zahtev za promenu termina je poslat.");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-100 text-slate-900">
      ...
      <main className="max-w-7xl mx-auto px-6 pb-16">
        <section className="bg-white rounded-[2rem] shadow-xl p-8 mb-8">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8 items-center">
            <div>
              ...
              <p className="text-slate-600 mt-4 text-base sm:text-lg max-w-2xl">
                Ovde možeš da pratiš svoje zahteve, potvrđene časove i detalje
        ...
        <section className="bg-white rounded-[2rem] shadow p-6 mb-8">
          <div className="mb-5">
            ...
            <p className="text-slate-500">
              Potvrđeni časovi koje ti je ski škola dodelila.
            </p>
          </div>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:justify-between gap-4">
                    <div>
                      ...
                      <p className="text-slate-500">
                        Instruktor: {lesson.instructor_name}
                      </p>
                    </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-slate-700">
                    ...
                    <p>
                      <b>Paket:</b> {packageLabel(lesson.group_package)}
                    </p>

                    <p>
                      <b>Nivo:</b> {lesson.client_skill_level}
                    </p>

                    <p>
                      <b>Prvi put:</b> {lesson.first_time ? "Da" : "Ne"}
                    </p>
                  </div>
                  ...
                  {changeLessonId === lesson.id ? (
                    <div className="mt-5 bg-blue-50 rounded-3xl p-5">
                      ...
                      <div className="flex flex-col sm:flex-row gap-3 mt-4">
                        <button
                          onClick={() => sendChangeRequest(lesson.id)}
        ...
        <section className="bg-white rounded-[2rem] shadow p-6 mb-8">
          <div className="mb-5">
            ...
            <p className="text-slate-500">
              Ovde vidiš status zahteva za promenu termina i odgovor bookera.
            </p>
          </div>

          {changeRequests.length === 0 ? (
            <div className="bg-slate-50 rounded-3xl p-8 text-center text-slate-500">
              Nemaš poslatih zahteva za promenu termina.
            </div>
          ) : (
              ...
              {changeRequests.map((changeRequest) => (
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div>
                      <span
                        className={`inline-block mb-2 px-3 py-1 rounded-full text-sm font-bold ${changeRequest.status === "pending"
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5">
                    <div className="bg-slate-50 rounded-2xl p-4">
                      ...
                      <p className="text-slate-700">
                        {formatTime(changeRequest.start_time)}–{formatTime(changeRequest.end_time)}
                      </p>
                    </div>
        ...
        <section className="bg-white rounded-[2rem] shadow p-6">
          <div className="mb-5">
            ...
            <p className="text-slate-500">
              Zahtevi koje si poslao ski školi i njihov trenutni status.
            </p>
          </div>
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                    <div>
                      <div className="flex flex-wrap gap-2 mb-2">
                        <span
                          className={`px-3 py-1 rounded-full text-sm font-bold ${request.status === "pending"
                            ? "bg-yellow-100 text-yellow-700"
                    ...
                    <div className="text-left md:text-right text-slate-600">
                      <p>
                        <b>Željeni datum:</b> {formatDate(request.preferred_date)}
                      </p>

                      <p>
                        <b>Željeno vreme:</b> {formatTime(request.preferred_time)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3 text-sm text-slate-700">
                    <p>
                      <b>Broj časova:</b> {request.number_of_lessons}
                    </p>

                    <p>
                      <b>Paket:</b> {packageLabel(request.group_package)}
                    </p>

                    <p>
                      <b>Prvi put:</b> {request.first_time ? "Da" : "Ne"}
                    </p>

                    {request.parent_name && (
                      <p>
                        <b>Roditelj:</b> {request.parent_name}
                      </p>
                    )}
                  </div>
